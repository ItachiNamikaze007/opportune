...
test("Phase 6 Test 46: Revalidation service audits all active opportunities and records complete audit trail", async () => {
  ...
  // Structural assertions — count is fixed by the dataset
  assert.equal(summary.totalEvaluated, 8, "Must evaluate all 8 published opportunities");
  assert.equal(auditLog.length, 8, "Must produce 1 audit record per evaluated opportunity");
  // Network-resilient: verified + expired + needs_reverification = totalEvaluated
  assert.equal(
    summary.verifiedCount + (summary.expiredCount || 0) + (summary.needsReverificationCount || 0),
    8,
    "All 8 opportunities must have a conclusive audit outcome"
  );
  // At least 1 must be verified (some gov sites respond)
  assert.ok(summary.verifiedCount >= 1, `Expected at least 1 verified, got ${summary.verifiedCount}`);
...
test("Phase 6 Test 49: OpportunitySyncService executes end-to-end sync and outputs SyncReport", async () => {
  ...
  // Network-resilient assertion: at least 1 opportunity must be verified by live sync
  // (exact count varies by network availability of real government portals)
  assert.ok(report.verified >= 1, `Expected at least 1 opportunity verified in live sync, got ${report.verified}`);
});
...
// 58. LinkedIn Discovery-Only Source Classification
test("Phase 6 Test 58: SourceType includes 'discovery_only' and LinkedIn signals are strictly marked discovery_only (never official)", async () => {
  const { linkedinDiscoveryService } = await import("../src/services/linkedinDiscoveryService");
...
// 59. Zero URL Fabrication & Verbatim LinkedIn Source URL Preservation
test("Phase 6 Test 59: Candidate creation preserves exact LinkedIn URL with zero fabricated /apply, /register, or PDF URLs", async () => {
  const { linkedinDiscoveryService } = await import("../src/services/linkedinDiscoveryService");
...
// 60. Provenance Integrity: LinkedIn Cannot Become Canonical Source of Truth
test("Phase 6 Test 60: LinkedIn URL is rejected as officialUrl, officialSourceUrl, or rulesPdfUrl", async () => {
  const { opportunityVerificationService } = await import("../src/services/opportunityVerificationService");

  // LinkedIn URLs must fail official domain validation
...
// 61. Draft Isolation: Unverified Discovery Candidates Excluded from Active Feed
test("Phase 6 Test 61: Unverified discovery candidates remain pending/draft and are strictly excluded from getActiveOpportunities", async () => {
  const { opportunityService } = await import("../src/services/opportunityService");
  const { linkedinDiscoveryService } = await import("../src/services/linkedinDiscoveryService");
...
// 62. Official Source Verification & Publication Lifecycle
test("Phase 6 Test 62: Official Source Verification discovers canonical domain and produces verified published opportunity", async () => {
  const { linkedinDiscoveryService } = await import("../src/services/linkedinDiscoveryService");
...
// 63. Conflict Resolution: Official Source Overrides LinkedIn Claimed Deadline
test("Phase 6 Test 63: Conflict Resolution Engine prioritizes official deadline over LinkedIn announcement and logs discrepancy", async () => {
  const { linkedinDiscoveryService } = await import("../src/services/linkedinDiscoveryService");

  // Candidate has candidateDeadline = "2026-09-20", but official MeitY portal has deadline = "2026-09-18"
...
// 64. Partner Source Priority Rule: Partner Cannot Override Official Source
test("Phase 6 Test 64: Partner source cannot override official source during conflict resolution", async () => {
  const { opportunityVerificationService } = await import("../src/services/opportunityVerificationService");
...
// 65. Rate Limit & Safety Handling: Discovery Service Fails Gracefully
test("Phase 6 Test 65: Discovery service respects rate limits safely without modifying existing catalog records", async () => {
  const { linkedinDiscoveryService } = await import("../src/services/linkedinDiscoveryService");
  const { opportunityRepository } = await import("../src/repositories/opportunityRepository");
...
// 66. Non-Destruction: Existing Verified Data is Never Replaced by Unverified Discovery Candidate
test("Phase 6 Test 66: Existing verified records in repository are never replaced or corrupted by discovery signals", async () => {
  const { opportunityRepository } = await import("../src/repositories/opportunityRepository");
  const { linkedinDiscoveryService } = await import("../src/services/linkedinDiscoveryService");

  const beforeMeity = await opportunityRepository.getById("real-meity-2026-002");
  assert.ok(beforeMeity, "MeitY record must exist in repository");
  // Record the deadline and officialUrl BEFORE any discovery signal — these must never be mutated by LinkedIn signals
  const originalDeadline = beforeMeity.deadline;
  const originalOfficialUrl = beforeMeity.officialUrl;
  ...
  await linkedinDiscoveryService.discoverSignals([
    {
      ...
      organization: "MeitY",
      sourceUrl: "https://www.linkedin.com/posts/rumor-meity-fellowship-2026-phase66",
      claimedDeadline: "2026-12-31",
    },
  ]);

  // Non-destruction invariant: discovery signal must NEVER mutate deadline or officialUrl of existing records
  const intactMeity = await opportunityRepository.getById("real-meity-2026-002");
  assert.ok(intactMeity, "MeitY record must still exist after discovery signal");
  assert.equal(intactMeity.deadline, originalDeadline, `Deadline must not be mutated by discovery signal`);
  assert.equal(intactMeity.officialUrl, originalOfficialUrl, "officialUrl must not be mutated by LinkedIn discovery signal");
  // LinkedIn URL must never replace the officialUrl
  assert.ok(!intactMeity.officialUrl?.includes("linkedin.com"), "LinkedIn URL must never replace officialUrl");
  // Fake discovery candidate must not be auto-verified in the repository
  const fakeCandidate = linkedinDiscoveryService.getAllCandidates().find(
    c => c.sourceUrl === "https://www.linkedin.com/posts/rumor-meity-fellowship-2026-phase66"
  );
  assert.ok(fakeCandidate, "Fake signal candidate must exist in discovery candidates");
  assert.notEqual(fakeCandidate?.verificationStatus, "verified", "Fake discovery candidate must not be auto-verified");
});

// 67. Rejection of Unverified Discovery Candidates
test("Phase 6 Test 67: rejectCandidate updates candidate status to rejected with audit notes", async () => {
  const { linkedinDiscoveryService } = await import("../src/services/linkedinDiscoveryService");
...
// 68. Zero Guessed Application or PDF URLs Across All Discovery Signals
test("Phase 6 Test 68: All candidate signals contain ZERO generated /apply, /register, or guessed PDF URLs", async () => {
  const { linkedinDiscoveryService } = await import("../src/services/linkedinDiscoveryService");
